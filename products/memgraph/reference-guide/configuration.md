...
| --query-modules-directory=/usr/lib/memgraph/query\_modules | Directory where modules with custom query procedures are stored. | `[string]` |
...
| --telemetry-enabled=true | Set to true to enable telemetry. We collect information about the running system \(CPU and memory information\)  and information about the database runtime \(vertex and edge counts and resource usage\) to allow for easier improvement of the product. | `[bool]` |
